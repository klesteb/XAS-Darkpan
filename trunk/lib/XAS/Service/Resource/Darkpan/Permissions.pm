package XAS::Service::Resource::Darkpan::Permissions;
...
use XAS::Service::Validate::Darkpan::Permissions;
...
sub init {
    ...
    my @fields = $self->processor->permissions->database->fields();

    $self->{'validate'} = XAS::Service::Validate::Darkpan::Permissions->new();
...
sub delete_resource {
    ...
    return $self->processor->permissions->remove($id);
...
sub get_response {
    ...
    my $build_data = sub {
        ...
        my $recs = $self->processor->permissions->search(-criteria => $criteria, -options => $options);

        while (my $datum = $recs->next) {

            my $rec = $self->build_response($datum);
            push(@{$data->{'_embedded'}->{'permissions'}}, $rec);
        ...
        if (my $valids = $self->validate->check($params)) {
            ...
            if ($action eq 'post') {
                ...
                if (defined($valids->{'cancel'})) {
                    ...
                    # redirect back to /api/permissions
...
sub build_20X {
    ...
    if (my $rec = $self->processor->permissions->find(-criteria => $criteria)) {
        
        my $info = $self->build_response($rec);
        $data->{'_embedded'}->{'permissions'} = $info;
...
sub post_data {
    ...
    my $results = $self->processor->permissions->add(
...
sub put_data {
    ...
    my $results = $self->processor->permissions->update(
        -id      => $params->{'id'},
        -pauseid => $params->{'pauseid'},
        -module  => $params->{'module'},
        -perms   => $params->{'perms'},
...
sub build_response {
    ...
    my $data = {
        _links => {
            self   => { href => "/api/permissions/$id", title => 'Self' },
            delete => { href => "/api/permissions/$id", title => 'Delete' },
            update => { href => "/api/permissions/$id", title => 'Update' },
        }
    };
...
sub create_form {
    ...
    # pauseid: the permissions PAUSE name
    # name:    full name
...
XAS::Service::Resource::Darkpan::Permissions - Perl extension for the XAS environment

=head1 SYNOPSIS

 my $builder = Plack::Builder->new();

 $builder->mount('/api/permissions' => Web::Machine->new(
...
provides a link to "/api/permissions" and the services it provides.
...
The processor used to manage the permissions table.
...
This returns the handle to access the permissions functionality.
